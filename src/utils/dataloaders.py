...
def _truncated_deepsea_decoder(example):
    schema = {
        'length':tf.io.FixedLenFeature([], tf.int64),
        'depth':tf.io.FixedLenFeature([], tf.int64),
        "num_labels":tf.io.FixedLenFeature([], tf.int64),
    ...
    content = tf.io.parse_single_example(example, schema)
    length, depth, num_labels = content['length'], content['depth'], content['num_labels']
    x = tf.io.parse_tensor(content['x'], out_type=tf.int8)
    x = tf.reshape(x, shape=(length, depth))
    x = tf.cast(x, tf.float32)

    y = tf.io.parse_tensor(content['y'], out_type=tf.int8)
    y = tf.reshape(y, shape=(num_labels,))
    y = tf.cast(y, dtype=x.dtype)
    return  x, y
...
def get_truncated_deepsea_dataset(datadir, compression_type='GZIP'):
    """
    datadir <str> -> The directory containing the train, test, validation splits
    of the truncated deepsea data.
    ...
    for split in splits:
        dataset[split] = _get_tfr_data(
                                datadir=os.path.join(datadir, split),
                                compression_type=compression_type
                                    ).map(_truncated_deepsea_decoder)
    return dataset

def get_deepsea_pretrained_logits(datadir, compression_type='GZIP'):
    """
    datadir <str> -> The directory containing the train, test, validation splits
    of the deepsea data.
    ...
    for split in splits:
        dataset[split] = _get_tfr_data(
                                datadir=os.path.join(datadir, split),
                                compression_type=compression_type
                                    ).map(_deepsea_pretrained_logits_decoder)
    return dataset
